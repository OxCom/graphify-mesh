...
def build_sdk_server(mesh: GraphifyMeshServer) -> Server:
    ...
    async def list_tools() -> list[types.Tool]:
        ...
        # `_handle_request` catches an uncaught handler exception and
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> types.CallToolResult:
        ...
        # SDK's own fallback (`Server.call_tool`'s `except Exception as e:
        # return self._make_error_result(str(e))`) puts the raw exception
        # text in the client-visible result — this catches first so a bug
        # here never leaks a traceback, a path, or an exception class name.
        try:
            result = await anyio.to_thread.run_sync(
                mesh.call_tool, name, arguments or {}, limiter=tool_limiter()
            )
        except Exception:
            log.exception("mesh.call_tool raised unexpectedly for tool %r", name)
            return types.CallToolResult(
                content=[types.TextContent(type="text", text="internal error")], isError=True
            )
